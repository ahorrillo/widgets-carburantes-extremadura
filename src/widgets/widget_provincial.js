...
export async function renderWidgetProvincial(containerId, provincia = 'badajoz', baseUrl = '') {
  ...
  try {
    ...
    const titulo = provincia.charAt(0).toUpperCase() + provincia.slice(1);
    const fechas = data.map(d => d.fecha);
    const gasolina = data.map(d => d[provincia]?.gasolina95_E5);
    const gasoleo = data.map(d => d[provincia]?.gasoleoA);

    container.innerHTML = `
      <div style="font-family: system-ui, sans-serif; border: 1px solid #e2e8f0; border-radius: 8px; padding: 16px; background: #fff;">
        <h4 style="margin: 0 0 12px 0; color: #0f172a;">Evolución de carburantes en ${titulo}</h4>
